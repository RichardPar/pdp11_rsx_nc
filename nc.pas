...
{ NC -- a Norton Commander style file manager for RSX-11M-PLUS

  Oregon Software Pascal-2 V2.1, VT100/VT200 (ANSI) terminal.
  ...
  Keys:
    ...
    Tab                          switch panels
    Enter                        enter directory, run .TSK, @ .CMD,
                                 view other files, or execute the
                                 command line if one has been typed
    Insert or ^T                 select / deselect file
  ...
  if c = 27 then
    ...
    if (c = ord('[')) or (c = ord('O')) then
      ...
      if (c >= ord('0')) and (c <= ord('9')) then
        ...
        else if n = 28 then k := kf1
        else if n = 29 then k := kf9
        end
...
{ run one MCR command line, showing it first }
var
  st: integer;
begin
  putstr(c);
...
{ make the terminal's default directory follow the active panel }
var
  t, c: str;
  st: integer;
begin
...
begin
  ...
  if st <> 1 then
    ...
    gotoxy(scrh, 1);
    setattr(akeyl);
    sset(t, ' Up Down PgUp PgDn Home End Left Right    ');
...
begin
  termreset;
  clrscr(adlg);
  l(1, 'NC -- Norton Commander for RSX-11M-PLUS (Pascal-2)');
  l(3, 'Up Down PgUp PgDn Home End  move      Left Right  page');
  l(4, 'Tab       switch panels           ^U  swap panels');
  l(5, 'Enter     enter directory / run .TSK / @.CMD / view file');
  ...
  l(16, 'digit: ESC 1 = F1 ... ESC 0 = F10.');
  l(18, 'Commands run through MCR in the active panel''s directory');
  l(19, '(the terminal default is changed with SET /DEF).');
  l(21, 'Press any key.');
  k := getkey;
  redraw
end;
